...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		for (const auto& node: GetAllNodes())
		{
			...
			{				
				float tempInfluance = GetNode(connection->GetTo())->GetInfluence() * expf(-connection->GetCost() * m_Decay);
				if (abs(tempInfluance) > abs(newInfluence))
				{
					newInfluence = tempInfluance;
				}
			}
			m_InfluenceDoubleBuffer.push_back((Lerp(newInfluence,node->GetInfluence(), m_Momentum)));
		}
		...
		for (const auto& node : GetAllNodes())
		{
			node->SetInfluence(m_InfluenceDoubleBuffer[counter]);
			++counter;
		}
	}
