...
namespace Elite
{
	...
	inline std::vector<T_NodeType*> EulerianPath<T_NodeType, T_ConnectionType>::FindPath(Eulerianity& eulerianity) const
	{
		...
		if (oddCount != 2 && nodes.size() != evenCount)
		{
			return path;
		}

		if (nodes.size() == evenCount)
		{
			startIdx = 0;
		}
		...
		while (nodeStack.size() != 0 && m_pGraph->GetNodeConnections(startIdx).size() > 0)
		{			
			if (m_pGraph->GetNodeConnections(startIdx).size() > 0)
			{	
				nodeStack.push(startIdx);
			
				int tempIndx{};
				tempIndx = graphCopy->GetNodeConnections(startIdx).front()->GetTo();
				graphCopy->RemoveConnection(startIdx, tempIndx);
	...
	inline void EulerianPath<T_NodeType, T_ConnectionType>::VisitAllNodesDFS(int startIdx, std::vector<bool>& visited) const
	{
		...
		for (auto connections : m_pGraph->GetNodeConnections(startIdx))
		{
			if (visited[connections->GetTo()] == false)
			{
				VisitAllNodesDFS(connections->GetTo(), visited);
			}
		}
