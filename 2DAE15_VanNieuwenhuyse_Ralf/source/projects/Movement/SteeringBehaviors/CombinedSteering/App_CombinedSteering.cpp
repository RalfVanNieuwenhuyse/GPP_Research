...
void App_CombinedSteering::Start()
{
	...
	m_pFleeingAgent = new SteeringAgent();
	m_pFleeingAgent->SetSteeringBehavior(m_pPrioritySteering);
	m_pFleeingAgent->SetMaxLinearSpeed(15.f);
	m_pFleeingAgent->SetMass(0.f);
	m_pFleeingAgent->SetAutoOrient(true);
...
void App_CombinedSteering::Update(float deltaTime)
{
	...
	m_pFleeingAgent->Update(deltaTime);
	m_pFleeingAgent->SetRenderBehavior(m_CanDebugRender);

	if (m_TrimWorld)
	{
		m_pDrunkAgent->TrimToWorld(m_TrimWorldSize);
		m_pFleeingAgent->TrimToWorld(m_TrimWorldSize);
	}
}
...
void App_CombinedSteering::Render(float deltaTime) const
{
	...
	m_pFleeingAgent->Render(deltaTime);
