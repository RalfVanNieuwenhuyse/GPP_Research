...
void App_GraphTheory::Update(float deltaTime)
{
	...
	Eulerianity eulerianty = eulerFinder.IsEulerian();

	
	for (size_t i = 0; i < eulerFinder.FindPath(eulerianty).size(); i++)
	{
		std::cout << eulerFinder.FindPath(eulerianty)[i];
		std::cout << '\n';
		int p{};
	}	
...
{
	m_GrapRenderer.RenderGraph(m_pGraph2D,true,true);
}
